...
# Ensure the static directory exists. In some deployment environments
# (e.g., when files have been uploaded via a UI or extracted manually),
# the static directory may be missing. Creating it here prevents
# `RuntimeError: Directory ... does not exist` when mounting static files.
try:
    if not static_path.exists():
        static_path.mkdir(parents=True, exist_ok=True)
except Exception:
    # If directory creation fails for any reason, fall back to None so
    # StaticFiles doesn't attempt to mount an invalid path.
    static_path = None
