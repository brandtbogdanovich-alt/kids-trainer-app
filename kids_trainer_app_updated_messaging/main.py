...
def create_tables() -> None:
    ...
    # Attempt to add new columns to the trainers table if they do not exist.
    # SQLite will raise an OperationalError if the column already exists, which we silently ignore.
    try:
        cursor.execute("ALTER TABLE trainers ADD COLUMN city TEXT NOT NULL DEFAULT ''")
    except sqlite3.OperationalError:
        pass
    try:
        cursor.execute("ALTER TABLE trainers ADD COLUMN rating REAL NOT NULL DEFAULT 0.0")
    except sqlite3.OperationalError:
        pass
...
def list_trainers(request: Request):
    """List registered trainers with optional filtering by sport and city.

    Query parameters:
        sport: partial match on the sport/discipline field
        city: partial match on trainer location
    """
    # Read query parameters for filtering
    sport_filter: str | None = request.query_params.get("sport")
    city_filter: str | None = request.query_params.get("city")
    conn = get_db()
    cursor = conn.cursor()
    # Build dynamic SQL query based on provided filters
    base_query = "SELECT * FROM trainers WHERE 1=1"
    params: list[str] = []
    if sport_filter:
        base_query += " AND sport LIKE ?"
...
async def register_trainer(request: Request):
    ...
    conn = get_db()
    cursor = conn.cursor()
    # Assign a default rating of 5.0 to newly registered trainers so they appear credible on listing pages.
    default_rating = 5.0
    cursor.execute(
        """
        INSERT INTO trainers (name, sport, credentials, bio, price, email, phone, city, rating)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
        (name, sport, credentials, bio, price_val, email, phone, city, default_rating),
    )
    conn.commit()
    conn.close()
...
# -----------------------------------------------------------------------------
# Additional routes inspired by full‑featured marketplaces like CoachUp, Care.com
# and UrbanSitter. These provide richer trainer profiles and support a search
# experience that parents expect from modern two‑sided marketplaces.

@app.get("/trainer/{trainer_id}")
def trainer_detail(trainer_id: int, request: Request):
    """Display a detailed profile page for a single trainer.

    If the trainer does not exist, redirect back to the trainer list.
    """
    conn = get_db()
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM trainers WHERE id = ?", (trainer_id,))
    ...
    return templates.TemplateResponse(
        "trainer_detail.html",
        {"request": request, "title": trainer["name"], "trainer": trainer},
    )